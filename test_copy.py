...
import test_dataloader as test_dataloader
from utils import SSIM,normalize,batch_PSNR
from torch.utils.data import DataLoader
import yaml
from yaml import Loader
from test_dataloader import progress
...
def test(model,data_name,test_all):
    ...
    test_loader = DataLoader(dataset=data_set, batch_size=10, shuffle=False, num_workers=4, drop_last=False  )
    model = model.cuda()
    # 判断加载 单个模型/最后一个模型
    if test_all == True:
    ...
    for name_index, load_name in enumerate(load_names):
        model.load_state_dict(torch.load(load_name,map_location='cuda:0'))
        ...
        with torch.no_grad(): 
            ...
            for rainy, gt in tqdm.tqdm(test_loader):
                ...
                if 0: # 输出图像
                    x = utils.make_grid(torch.cat((out,gt,torch.squeeze(rainy,0))))
...
if __name__ == "__main__":
    ...
    count = 0
    #  只需要输入
    res += test(model,opt.data_name,True)
    print(res)    
